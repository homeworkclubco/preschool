...
1. **Read the CSS file** and identify:
   ...
   - Token scales and patterns (e.g., `--spacing-xs` through `--spacing-xl`)
...
<Iframe>
  <div class="stack">
    <div style="background: var(--color-gray-200); height: var(--spacing-xs); width: 100%;"></div>
    <div>--spacing-xs: {value}</div>

    <div style="background: var(--color-gray-200); height: var(--spacing-sm); width: 100%;"></div>
    <div>--spacing-sm: {value}</div>
...
```css
--spacing-md-lg: clamp(1rem, 2vw, 1.5rem);
```
...
|-------|-------|--------|-------|
| `--spacing-xs` | `0.25rem` | [8px bar] | Tight spacing |
| `--spacing-sm` | `0.5rem` | [16px bar] | Related items |
```

## Extraction Logic

1. **Parse all `--token-name: value;` declarations**
2. **Group by prefix** (e.g., all `--color-*`, all `--spacing-*`)
